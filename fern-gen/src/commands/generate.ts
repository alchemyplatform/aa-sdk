...
async function generateDocumentation(
  ...
  const node = resolveReExport(sourceFilePath, importedName) as unknown as Node; // TODO: fix this. Type should be the same as it is in doc-gen
  if (!node) {
    return;
  }
  ...
  if (ts.isClassDeclaration(node)) {
    ...
    generateFunctionDocs(
      node as ts.VariableStatement | ts.FunctionDeclaration | ts.ClassElement,
      importedName,
      outputFilePath,
      packageName,
      isTsx
