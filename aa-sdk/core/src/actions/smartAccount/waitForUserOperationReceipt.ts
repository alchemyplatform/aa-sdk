...
import { clientHeaderTrack, type UserOperationReceipt } from "../../index.js";
 ...
 *
 * // smart account client is already extended with waitForUserOperationReceipt
 ...
 * @returns {Promise<UserOperationReceipt>} A promise that resolves to the transaction receipt when the transaction is confirmed
 */
...
>(
...
) => Promise<UserOperationReceipt> = async (client_, args) => {
  const client = clientHeaderTrack(client_, "waitForUserOperationReceipt");
  ...
  for (let i = 0; i < retries.maxRetries; i++) {
    ...
    const receipt = await client
      ...
      .catch((e) => {
        Logger.error(
          `[SmartAccountProvider] waitForUserOperationReceipt error fetching receipt for ${hash}: ${e}`,
        );
      });
