...
import { alchemyGasAndPaymasterAndDataHooks } from "./alchemyGasManagerHooks.js";
import { toHex, type Address } from "viem";

describe("alchemyGasAndPaymasterAndDataHooks - Optimized Flow", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  it("should make single RPC call and cache results", async () => {
    const policyId = "test-policy-optimized";
    const hooks = alchemyGasAndPaymasterAndDataHooks(policyId);
  ...
  it("should use cached fee estimates", async () => {
    const hooks = alchemyGasAndPaymasterAndDataHooks("test-policy");
