...
import { getSigner, type AlchemySigner } from "@account-kit/core";
import { watchSigner } from "../core/watchSigner.js";
 ...
 *
 * @returns {RNAlchemySignerType | null} The current Alchemy signer or null if none is available. [ref](https://github.com/alchemyplatform/aa-sdk/blob/main/account-kit/signer/src/client/index.ts#L50)
 *
 * @example
 * ```ts twoslash
...
export const useSigner = (): AlchemySigner | null => {
  ...
  return useSyncExternalStore(
    watchSigner(config),
    () => {
      const signer: AlchemySigner | null = getSigner(config);
      if (signer == null) {
        throw new Error("Signer not found");
      }
      if ("type" in signer && signer.type === "RNAlchemySigner") {
        return signer;
      }

      throw new Error("Signer is not an instance of RNAlchemySignerType");
    },
