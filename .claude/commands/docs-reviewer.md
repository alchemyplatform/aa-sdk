...
- **No arguments (default)**: Automatically detect and process all MDX files that have been modified, added, or renamed in the current git branch compared to the v5 branch using `git diff --name-only --diff-filter=AMR origin/v5.x.x...HEAD -- '*.mdx'`
