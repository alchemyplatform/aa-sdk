---
title: ValidationDataView
description: Overview of ValidationDataView
slug: wallets/reference/account-kit/core/src/type-aliases/ValidationDataView
layout: reference
---

```ts
type ValidationDataView = object;
```

Defined in: account-kit/smart-contracts/dist/types/src/ma-v2/account/common/modularAccountV2Base.d.ts:16

## Properties

<table>
  <thead>
    <tr>
      <th align="left">Property</th>
      <th align="left">Type</th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        <a id="executionhooks" /> `executionHooks`
      </td>

      <td>
        readonly `Hex`\[]
      </td>
    </tr>

    <tr>
      <td>
        <a id="selectors" /> `selectors`
      </td>

      <td>
        readonly `Hex`\[]
      </td>
    </tr>

    <tr>
      <td>
        <a id="validationflags" /> `validationFlags`
      </td>

      <td>
        `number`
      </td>
    </tr>

    <tr>
      <td>
        <a id="validationhooks" /> `validationHooks`
      </td>

      <td>
        readonly `Hex`\[]
      </td>
    </tr>

  </tbody>
</table>
