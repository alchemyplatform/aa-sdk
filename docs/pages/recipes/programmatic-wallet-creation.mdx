...
```
ALCHEMY_API_KEY=YOUR_API_KEY # Get your app API key: https://dashboard.alchemy.com/apps
ALCHEMY_POLICY_ID=YOUR_POLICY_ID # Get your gas sponsorship policy ID: https://dashboard.alchemy.com/services/gas-manager/configuration
```
...
You can use email/OTP, passkeys or social login (when getting a signer through frontend, see [react quickstart](https://www.alchemy.com/docs/wallets/react/quickstart)), EOAs, KMS/HSM, or custodial signers. For simplicity here, we generate a private key with `viem` and wrap it with `LocalAccountSigner`:
...
## Step 6 — Prepare → sign → send a sponsored call
...
const prepared = await client.prepareCalls({
  from: address,
  calls: [{ to: "0x0000000000000000000000000000000000000000", data: "0x", value: "0x0" }], // minimal call to deploy the account contract
  capabilities: {
...
> Persist `address`, `preparedCallIds`, `txHash`, and `chainId` (and store only a signer locator in production—never raw keys).

---

## Optional: Non-sponsored path

Remove the `paymasterService` capability in `prepareCalls` and **fund the account** to pay gas. The rest of the flow is unchanged.
...
And when opened in an Arbitrum Sepolia explorer, you should see the [deployment](https://sepolia.arbiscan.io/address/0x022fA5358476f0EB881138BD822E5150AFb36c5B#internaltx) 🎉
