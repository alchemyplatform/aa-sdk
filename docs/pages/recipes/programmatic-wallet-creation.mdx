...
<Note>For non-sponsored path, remove the `paymasterService` capability in `prepareCalls` and **fund the account** to pay gas. The rest of the flow is unchanged.</Note>
