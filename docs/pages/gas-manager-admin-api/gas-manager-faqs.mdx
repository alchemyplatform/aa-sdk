...
## Sending User Operations without Sponsoring Gas

### How can I send User Operations without Sponsoring Gas?

Sponsoring gas for your users is optional. If you prefer for users’ smart accounts to pay gas themselves in the native token, you have two options:

**1. Omit the gas policy in your config**  
...
**2. Override sponsorship at the UserOp level**  
Explicitly override sponsorship by setting `paymasterAndData` to `0x`:
...
If you are not sponsoring gas, the user’s smart account must have enough native tokens (e.g., ETH on Base) to cover gas fees.
