...
```bash yarn
yarn add @account-kit/infra @account-kit/smart-contracts @aa-sdk/code viem
```

```bash npm
npm i -s @account-kit/infra @account-kit/smart-contracts @aa-sdk/code viem
```
...
<Warning>
  Replace `ALCHEMY_API_KEY` and `POLICY_ID` with the key and Policy ID created
  above.
</Warning>
