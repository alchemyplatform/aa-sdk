...
To bring your own signer, you'll need to define a `SmartAccountSigner` interface that implements `signAuthorization`. See the details for the interface requirements [here](/wallets/third-party/signers#implementing-smartaccountauthenticator-or-smartaccountsigner). For example, you can extend a Wallet Client to include signAuthorization, if supported by the signer, like so:
...
const embeddedWallet = window.ethereum; // or any other EIP-1193 provider
const baseSigner = new WalletClientSigner(
createWalletClient({
account: embeddedWallet!.address as Hex,
chain: sepolia
transport: custom(embeddedWallet),
}),
"test"
);
