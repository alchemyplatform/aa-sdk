...
<Tabs>
  <Tab title="React">
    ...
    - `accountAuthMode` (optional) - `'eip7702'` (default, recommended) or `'owner'` for traditional smart accounts
    ...
    - **`'eip7702'` (default, recommended)**: Uses EIP-7702 to delegate the Privy EOA to a smart account. No deployment needed, funds stay at the EOA address.
    ...
    **Getting the Smart Account Address (owner mode):**
    ...
    - **EVM**: EIP-7702 (default) delegates your EOA to a smart account at send time. Alternatively, use `accountAuthMode="owner"` for traditional smart accounts.
