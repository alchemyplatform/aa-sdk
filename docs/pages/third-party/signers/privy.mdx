...
<Tabs>
  <Tab title="React">
    ...
    - `walletAddress` (optional) - specify which wallet address to use if user has multiple wallets (defaults to first embedded wallet)
  ...
  <Tab title="React Native (Expo)">
    ...
    **Important:** Import from `@account-kit/privy-integration/react-native` for React Native apps.
    ...
    function SendButtons() {
      ...
      return (
        <>
          <Button onPress={single} disabled={isLoading} title="Send" />
          <Button onPress={batch} disabled={isLoading} title="Send Batch" />
        </>
      );
    }
    ```
    ...
    function SwapButton() {
      ...
      return <Button onPress={handleSwap} disabled={isLoading} title="Swap" />;
    }
    ```
    ...
    function Advanced() {
      ...
      return <Button onPress={doOp} title="Advanced Op" />;
    }
    ```
