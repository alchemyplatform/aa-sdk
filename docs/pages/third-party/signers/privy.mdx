...
<Tabs>
  <Tab title="React">
    ...
    Use Privy as your auth provider while upgrading EOAs to Smart Wallets via EIP-7702. The `@account-kit/privy-integration` package adds:

    - **EIP-7702 Delegation** to upgrade EOAs without migration
    - **Gas Sponsorship** via Gas Manager policies
    - **Token Swaps** using Alchemy's swap infra
    - **Batched Transactions** with a single call
    ...
    ### 0. Get your keys (API key, Policy ID, Privy App ID)
    ...
    ### 1. Wrap Providers (PrivyProvider → AlchemyProvider)
    ...
    - For keys: get Alchemy API key, Gas Manager policy (optional), and Privy App ID.
