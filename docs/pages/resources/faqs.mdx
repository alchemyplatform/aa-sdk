...
<Accordion title="Answer">
    ...
    4. If you need to make subsequent requests, to avoid calling the whoami endpoint on every request. After verifying the `whoami`, the backend can issues its own session token (e.g. an HTTP-only cookie or access token). If the token is present, you can safely skip the `whoami` check.
    ...
    **Why this approach?**
    ...
    ### Option 2: SIWE [Sign-In With Ethereum](https://eips.ethereum.org/EIPS/eip-4361)
