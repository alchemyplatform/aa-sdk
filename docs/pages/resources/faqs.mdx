...
<Accordion title="Answer">
    ...
    ### Option 1: `stampWhoAmI` + `whoami` (Recommended for SCAs)

    Use this flow when working with Smart Contract Accounts (SCAs) via Account Kit.

    1. The frontend generates a stamped identity proof using [signer.inner.stampWhoAmI](https://www.alchemy.com/docs/wallets/reference/account-kit/signer/classes/BaseSignerClient/stampWhoami).
    ...
    4. If you need to make subsequent requests, to avoid calling the whoami endpoint on every request. After verifying the `whoami`, the backend can issues its own session token (e.g. an HTTP-only cookie or access token). If the token is present, you safely skip the `whoami` check.

    **Why this approach?**
    - Trusted and cryptographically secure
    - No user signature required
    - **Cheaper** than flows requiring a signed message

    ---

    ### Option 2: SIWE [Sign-In With Ethereum](ERC-4361: Sign-In with Ethereum)

    Use this flow if you're working with EOAs, or if you want a unified flow for EOAs and SCAs.
    ...
    **Note:**
    ...
    - Required for EOAs, optional for SCAs.
...
However, we have created a small PoC using Expo that you can find [here](https://github.com/alchemyplatform/aa-sdk-rn-expo/tree/main). For more information on how to use Account Kit within a React Native application see [the guide](/wallets/react-native/overview).
