...
You can allow users to login by connecting their existing EOA using Account Kit. This can be done using our pre-built UI components or custom UI - no need for a separate wallet connector!
Depending on what method you choose, you can bring in your EOA:

- As a "pure" EOA, meaning users will use their EOA and the funds in that wallet as normal. Since it is an EOA and not a smart account, it will not have access to AA features like gas sponsorship or batching.
- As an owner and signer of a new smart account, meaning users will maintain self-custody and sign with their EOA for each transaction. They will not be able to use funds from the EOA directly, but their new wallet will get AA features like sponsorship and batching.
...
### Bring in an EOA as an Owner

For local wallets or JSON-RPC wallets that support the [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193) standard (like MetaMask, Coinbase Wallet, etc.), you can use `WalletClientSigner` from `@aa-sdk/core`. More info [here](https://www.alchemy.com/docs/wallets/third-party/signers).
...
### Bring in a Local Account as an Owner
