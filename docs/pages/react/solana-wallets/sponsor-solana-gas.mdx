...
You can sponsor gas either globally (for all transactions) or on a per-transaction basis.
...
If you want to apply a sponsorship policy to all transactions sent by the Solana wallet provider, you can configure gas sponsorship at the Alchemy configuration level by passing a policyId in to your Solana config.
