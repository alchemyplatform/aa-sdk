...
async function useSessionKey(sessionKey: LocalAccountSigner, permissions: any) {
  try {
    // Request account
    const account = await client.requestAccount();
...
async function main() {
  ...
  await useSessionKey(sessionKey, permissions);
}
```
        ...
        },
        {
          ...
          data: encodeFunctionData({
            ...
            args: [
              ...
              [DEMO_USDC_ADDRESS, "0x0000000000000000000000000000000000000000"], // path
              account.address, // to
...
async function main() {
  ...
  await performAutomatedSwap(sessionKey, permissions);
}
```
