...
<Tabs>
  ...
  <Tab title="APIs">
    It’s easy to send calls in batch from a single account! Simply include an array of calls when calling `wallet_sendPreparedCalls`. The calls will be batched into a single atomic user operation.
