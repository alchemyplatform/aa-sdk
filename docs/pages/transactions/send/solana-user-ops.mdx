...
Once you've [created](/wallets/react/solana-wallets/get-started) a smart wallet on Solana, [send transactions](/wallets/transactions/send/send-solana-user-operations#send-a-single-transaction) and [sign messages](/wallets/transactions/send/send-solana-user-operations#sign-messages) using the `useSolanaTransaction` and `useSolanaSignMessage` hooks. You can also add [multiple signers](/wallets/transactions/send/send-solana-user-operations#multi-signature-support-using-custom-signers) to your Solana wallet.
