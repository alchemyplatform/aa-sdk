...
<Tabs>
  ...
  <Tab title="APIs">
    When sending a user operation with Wallet APIs, the only thing you need to do to sponsor gas is add your Policy ID to your request params at `capabilites.paymasterService.policyId` when making a `wallet_prepareCalls` call.

    <Tip>
    If you are unfamiliar with sending an unsponsored user operation using Wallet APIs, check out [this guide](/wallets/transactions/send/send-user-operations) first to learn how to request a smart account and sign calls.
    </Tip>
