...
If you're using React, get started with Solana wallets [here](/docs/wallets/signer/solana-wallets/solana-signer-package).
...
1. Set up your [sponsorship policy](https://dashboard.alchemy.com/signup) in the dashboard to retrieve a **policy ID**
...
async function signTransaction(
  ...
  const transaction = await signer.addSponsorship(
    instructions,
    connection,
    "<policyId>", // Replace with your solana sponsorship policy ID: https://dashboard.alchemy.com/signup
  );
