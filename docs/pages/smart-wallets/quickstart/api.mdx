...
If you are using a library such as Viem that supports the `personal_sign` method, you should use that to sign the hash `signatureRequest.data.raw` (since the `signatureRequest.type` is `"personal_sign"`).
