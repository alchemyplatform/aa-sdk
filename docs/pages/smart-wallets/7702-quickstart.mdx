...
<Tip title="Don't have an API key?" icon="star">
  Start using the Alchemy Wallets API today! [Get started for free](https://dashboard.alchemy.com/signup/?a=f8afc2202c).
</Tip>
...
```ts
// Request the account using the "7702" account type.
...
const userOpCall = preparedCalls.type === "array"
  ? preparedCalls.data.find((it) => it.type === "user-operation-v070")
  : preparedCalls

const authorizationCall = preparedCalls.type === "array"
