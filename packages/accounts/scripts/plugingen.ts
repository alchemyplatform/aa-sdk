  ...
  return {
    ...
    run: async ({ contracts }) => {
      ...
      for (const contract of contracts) {
        ...
        // create installPlugin helper function
        if (config.installConfig != null) {
          imports.push(dedent`
            import { ${
              config.installConfig.dependencies != null ? "encodePacked," : ""
          ...
          types.push(dedent`
            type InstallArgs = ${JSON.stringify(initArgs)};
            export type Install${contract.name}Params = {
      ...
      const uniqueDeps = Array.from(
        new Set(
          (config.installConfig?.dependencies ?? []).map((x) => x.plugin.name)
        )
      );

      uniqueDeps.forEach((x) =>
        imports.push(dedent`
        import { ${x} } from "../${kebabCase(
          x.replaceAll(/[pP]lugin/g, "")
        )}/plugin.js";
      `)
      );
