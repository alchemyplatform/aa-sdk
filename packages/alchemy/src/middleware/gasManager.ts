 ...
 * @template {SmartContractAccount} TAccount
 ...
 * @param struct user operation struct
 * @param account smart contract account
...
>(
  ...
  return {
    ...
    ...(account.getEntryPoint().version === "0.7.0"
      ...
      : {
          paymasterAndData: `${paymasterAddress}${dummyData}` as Hex,
        }),
...
function requestGasAndPaymasterData<C extends ClientWithAlchemyMethods>(
  ...
  return {
    dummyPaymasterAndData: async (struct, { account }) =>
      dummyPaymasterAndData(client, config, struct, account),
