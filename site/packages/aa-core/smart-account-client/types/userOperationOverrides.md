...
These override values are available from each middleware of the `SmartAccountProvider`. For example, the default middlewares such as [`gasEstimator`](/packages/aa-core/smart-account-client/withGasEstimator.md) or [`feeDataGetter`](/packages/aa-core/smart-account-client/withFeeDataGetter.md) apply the overrides values to the estimated values if the override values are provided.
