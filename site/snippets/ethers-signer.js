...
const owner = LocalAccountSigner.mnemonicToAccountSigner(process.env.YOUR_OWNER_MNEMONIC);
const chain = polygonMumbai;
const entryPointAddress = getDefaultEntryPointContract(chain);
const factoryAddress = getDefaultLightAccountFactory(chain);
export const signer = provider.connectToAccount((rpcClient) => new LightSmartContractAccount({
    entryPointAddress,
    chain,
    factoryAddress,
    rpcClient,
