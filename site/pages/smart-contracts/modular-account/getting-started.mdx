...
Next, if you want to use a different `signer` with a smart account signer, check out [choosing a signer](/signer/what-is-a-signer). Otherwise, if you are ready to get on-chain, go to [send user operations](#TODO/smart-contracts/using-smart-accounts/send-user-operations).
