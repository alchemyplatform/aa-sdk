...
- Your user is already logged in with at least one authentication factor (e.g. [Email OTP](/signer/authentication/email-otp), [Social Login](/signer/authentication/social-login), etc.).
...
const { multiFactors } = await signer.inner.addMFA({
...
const { multiFactors } = await signer.inner.getMfaFactors();
```
