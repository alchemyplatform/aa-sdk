...
When MFA is enabled, the authentication process for social login works as follows:

1. The user initiates authentication with a social provider (Google, Facebook, etc.)
2. After successful authentication with the social provider
3. The user is prompted to enter the 6-digit code from their authenticator app in the OAuth callback page

The key advantage of social login with MFA is that the authenticator app verification happens automatically in the OAuth callback flow - your application code doesn't need special handling.

## Implementation

### Step 1: Set Up Social Login

First, implement social login as described in the [Social Login Authentication](/react/login-methods/social-login) guide. You can use either the pre-built UI components or custom UI approach.

### Step 2: Implement MFA Setup for Users

Before users can authenticate with social login + MFA, they need to set up an authenticator app for their account. Implement the MFA setup flow as described in the [Setting Up Multi-Factor Authentication](/react/mfa/setup-mfa) guide.
...
1. Set up multi-factor authentication for a user using an authenticator app (see [Setting Up Multi-Factor Authentication](/react/mfa/setup-mfa))
2. Log out the user
