...
```ts
import { type UseWaitForUserOperationTransactionResult } from "@account-kit/react";
```
...
```ts [types.ts]
// [!include ~/../aa-sdk/core/src/actions/smartAccount/types.ts:WaitForUserOperationTxParameters]
```
