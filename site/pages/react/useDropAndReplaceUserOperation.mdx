...
```ts
import { type UseDropAndReplaceUserOperationResult } from "@account-kit/react";
```
...
```ts [types.ts]
// [!include ~/../aa-sdk/core/src/client/types.ts:SendUserOperationResult]
```
...
```ts [types.ts]
// [!include ~/../aa-sdk/core/src/actions/smartAccount/types.ts:DropAndReplaceUserOperationParameters]
```
