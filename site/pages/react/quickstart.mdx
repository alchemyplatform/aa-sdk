...
Account Kit React is the easiest way to build with smart accounts. Craft delightful sign up flows with email and passkey to onboard new users, and then checkout with gas sponsorship using built-in AA. For existing EOA users, the modal also support injected wallets!

We have a TON more features coming soon:  WalletConnect, Social OAuth, SMS, and more.

This quickstart guide is includes two guides:
1. Starting with a new project using a NextJS template
2. Integrating Account Kit into an existing project

Both guides will walk you through the process of setting up a project and then integrating our UI Components for simple sign-up!
...
2. Create a **new account config** in your [Alchemy Accounts Manager Dashboard](https://dashboard.alchemy.com/accounts)
    1. Use [http://localhost:3000](https://localhost:3000/) as the “Redirect URL” for testing (***make sure to set http NOT https for local testing)
        <img src="/images/new-embedded-account.png" alt="alchemy create new embedded account config highlighting where to set localhost" />
        
    2. Optionally, stylize ✨ the auth email with your brand color and logo!
    3. Apply the config to your app from step 1
        <img src="/images/apply-config-to-app.png" alt="apply your the config to the app from the first step" />
...
<img src="/images/ui-component-demo-app.png" alt="Customize styles and auth methods in the demo app" />
