...
For more on tailwind setup, see these [steps](https://v3.tailwindcss.com/docs/installation/using-postcss).

:::tip

:::details[Using tailwind v4?]

If you are setting up a new app (or upgrading an existing app) and would like to use tailwind v4, it is possible with a couple of extra steps.

1. Follow the tailwind docs to [install](https://tailwindcss.com/docs/installation/using-postcss) or [upgrade](https://tailwindcss.com/docs/upgrade-guide) to tailwind v4.
2. Create a `tailwind.config.ts` file. This is not included in v4 by default, but is [still supported for backward compatibility](https://tailwindcss.com/docs/upgrade-guide#using-a-javascript-config-file).

```ts twoslash [tailwind.config.ts]
import { withAccountKitUi } from "@account-kit/react/tailwind";

const config = {
  // If you have any other legacy options, you can include them here.
  // If not, it's okay to leave empty.
};
export default withAccountKitUi(config, {
  // Account Kit UI config options (optional). See below for more info.
});
```

3. Update your `globals.css` file to include the config file you just created.

```css twoslash [globals.css]
@import "tailwindcss";

/* relative path to your config file */ // [!code ++]
@config '../../tailwind.config.ts'; // [!code ++]
/* required for account kit dark mode support */ // [!code ++]
@custom-variant dark (&:is(.dark, .dark *)); // [!code ++]
```
...
1. In the demo app, copy your `tailwind.config.ts` code into the file of the same name in your project. This will apply your custom styles: colors, border radius, and illustrations.
   ...
   2. Light/Dark mode:
      1. Light Mode and Dark Mode are set to match the system theme. You can manually override this by following [this](https://v3.tailwindcss.com/docs/dark-mode#toggling-dark-mode-manually) guide. TLDR: update your tailwindcss config to use `selector` mode for dark and then add the `dark` class to the root of your DOM
...
2. Make sure you wrap your existing tailwind config with `withAccountKitUi` :
   ...
   export default withAccountKitUi(
     {
       // 1. (required) your tailwind config generated in setup or existing config including content, theme, plugins, etc. - https://v3.tailwindcss.com/docs/installation/using-postcss
     },
     {
