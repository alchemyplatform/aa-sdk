...
```ts
import { type UseSmartAccountClientProps } from "@account-kit/react";
```
...
```ts [types.ts]
// [!include ~/../account-kit/smart-contracts/src/light-account/accounts/account.ts:CreateLightAccountParams]
```
...
```ts [types.ts]
// [!include ~/../account-kit/smart-contracts/src/msca/account/multiOwnerAccount.ts:CreateMultiOwnerModularAccountParams]
```
...
```ts [types.ts]
// [!include ~/../account-kit/infra/src/client/smartAccountClient.ts:AlchemySmartAccountClientConfig]
```
