...
```ts
import { type UseSendTransactions } from "@account-kit/react";
```
...
```ts [types.ts]
// [!include ~/../aa-sdk/core/src/client/types.ts:SendTransactionsResult]
```
...
```ts [types.ts]
// [!include ~/../aa-sdk/core/src/actions/smartAccount/types.ts:SendTransactionsParameters]
```
