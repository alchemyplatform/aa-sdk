...
We would go through the steps to get your environment setup for React Native development. This guide will focus on using Expo, but the same principles should apply to a bare React Native project as well.
...
```bash [npm]
npm install -s @alchemy/aa-accounts
```
