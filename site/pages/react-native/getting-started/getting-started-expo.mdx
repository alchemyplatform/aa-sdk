...
The first thing we need to do is make sure we're on the latest version of Expo (SDK 52 or later). The reason for this is that we need React Native version 0.74 or higher because it has `TextEncoder` natively supported.
