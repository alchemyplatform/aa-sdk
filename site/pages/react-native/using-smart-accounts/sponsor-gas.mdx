...
## Conditionally Bypass Gas Sponsorship

If you decide you don't want to sponsor gas for some user operations and bypass the paymaster, you can set this configuration using user operation overrides.

We can check we’re eligible for gas sponsorship first:

```tsx [gas-sponsorship.tsx]
...
const { eligible } = await accountClient.checkGasSponsorshipEligibility({
  uo: {
    data: "0xCalldata",
    target: "0xTarget",
    value: 0n,
...
If `eligible` is `false`, we can pass in a user operation override to bypass the [paymaster](https://github.com/alchemyplatform/aa-sdk/blob/main/aa-sdk/core/src/types.ts#L76):

```tsx [gas-sponsorship.tsx]
import { UserOperationOverrides } from "@aa-sdk/core";
import { createModularAccountV2Client } from "@account-kit/smart-contracts";
import { LocalAccountSigner } from "@aa-sdk/core";
import { alchemy, sepolia } from "@account-kit/infra";
import { generatePrivateKey } from "viem/accounts";
...
const userOperationResult = await accountClient.sendUserOperation({
  uo: {
...
And then you can pay for your own user operations.
:::
