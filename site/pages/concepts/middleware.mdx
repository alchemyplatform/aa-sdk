...
The [Smart Account Client](/concepts/smart-account-client) is extended with a series of a middleware. When sending or building user operations for signing and sending, the flow can be pretty involved.
...
If you want to sponsor gas, the Smart Account Client's in these packages abstract the relevant middleware away from you since all you need is a `policyId` to sponsore gas with Alchemy.
:::
