...
If you are looking to pay for gas permissionlessly using USDC, you can use [Circle's Paymaster](https://developers.circle.com/stablecoins/paymaster-overview). Here's an example using Circle Paymaster with the Alchemy SDK:
...
import {
  ...
  getContracts,
} from "circle-paymaster-aa-sdk-middleware";
...
if ((await usdcContract.read.balanceOf([account.address])) < 1000000) {
  console.log(
    `Fund ${account.address} with USDC from https://faucet.circle.com, then run this again.`
  );
  process.exit();
}
