...
export const useAccountPlugins = ({
  ...
  const availablePlugins = useMemo(() => ({
    [PluginType.MultiOwnerPlugin]: multiOwnerPlugin,
    [PluginType.SessionKeyPlugin]: sessionKeyPlugin,
    [PluginType.TokenReceiverPlugin]: tokenReceiverPlugin,
    ...
    async (pluginType: PluginType) => {
     switch (pluginType) {
      case PluginType.MultiOwnerPlugin:
        if (await multiOwnerPlugin?.isPluginInstalled()) {
          throw new Error(`Plugin ${Object.keys(PluginType).}`)
        }
      break
