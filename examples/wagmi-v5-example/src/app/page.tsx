...
export default function Home() {
  ...
  const isSmartWallet = connectorClient?.name === "alchemySmartWalletClient";
    ...
    <div className="flex flex-col items-center justify-items-center min-h-screen p-12 gap-6">
      ...
      {!account.isConnected ? (
        ...
        <>
          ...
          <WalletClientDemo />
          <SigningDemo />
          {isSmartWallet && <SendCallsDemo />}
          {isSmartWallet && <PrepareAndSendCallsDemo />}
          <SendTransactionDemo />
          {isSmartWallet && <SwapDemo />}
          <CapabilitiesDemo />
        </>
      )}
    </div>
...
const ConnectDemo = () => {
  ...
  return (
    <div className="flex flex-col gap-4 items-center">
      ...
      {error && (
        <p className="text-red-600 break-all max-w-xl">
          Error: {error.message}
        </p>
      )}
    </div>
...
const SwapDemo = () => {
  ...
  const {
    ...
    mutationFn: async (params: {
      ...
      return prepareSwap(config, params);
    },
  });
  ...
  const {
    ...
    mutationFn: async () => {
      if (!preparedSwap) throw new Error("No prepared swap");
      return sendPreparedCalls(config, preparedSwap);
    },
  });
  ...
  return (
    <div className="flex flex-col gap-2 items-center">
      ...
      {preparedSwap && <p className="text-green-600">Swap prepared!</p>}
      {prepareSwapError && (
        <p className="text-red-600">
          Error preparing swap: {prepareSwapError.message}
        </p>
      )}
      {submitSwapResult && (
...
const PrepareAndSendCallsDemo = () => {
  ...
  const {
    ...
    mutationFn: async () => {
      if (!preparedCalls) throw new Error("No prepared calls");
      return sendPreparedCalls(config, preparedCalls);
    },
  });
  ...
  return (
    <div className="flex flex-col gap-2 items-center">
      ...
      {preparedCalls && <p className="text-green-600">Calls prepared!</p>}
      {prepareCallsError && (
        <p className="text-red-600">
          Error preparing calls: {prepareCallsError.message}
        </p>
      )}
      {sendCallsResult && (
        <p className="break-all max-w-xl">Calls sent: {sendCallsResult.id}</p>
